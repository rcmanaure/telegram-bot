...
- [ ] **PORTAL-RLS-VERIFY (P1, blocks portal ship)** — Verify Postgres RLS coexists with raw-SQL retrieve_context, the SET LOCAL hnsw settings (no commit between SET LOCAL and SELECT), and the single connection pool. RLS session GUC must be set per request on the same connection that runs the vector SELECT. This is the riskiest integration in the portal phase; validate in eng review + integration test before relying on it. Effort: human ~1-2d / CC ~30min.
